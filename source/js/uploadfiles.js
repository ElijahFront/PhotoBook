...
$(document).ready(function() {
		...
		for (i = start; i < end; i++) {
			...
			if($('#dropped-files > .image').length <= maxFiles) {
				$('#dropped-files').append('' +
					'<li id="drop-'+i+'" class="image add_photos__item">' +
					'<div class="add_photos__img_wrap">' +
					'<div class="icon_wrap">' +
					'<svg role="img" class="svg-close-dims">' +
					'<use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="/assets/img/svg-sprite.svg#close"></use>' +
					'</svg>' +
					'</div><img src="'+dataArray[i].value+'" class="add_photos__img">' +
					'</div>' +
					'</li>');
			}
		}
	...
	$("#dropped-files").on("click","[id^='drop']", function() {
